...
//METODO PARA ATUALIZAR A SITUAÇÃO DE CADA DOCUMENTO 
	public function editarSituacao()
	{

		$sql = new Sql();

		$results = $sql->select("CALL sp_editar_situacaoT(:id_documento,:situacao_t)", [
			':id_documento'=>$this->getid_documento(),
...
public function deletarArquivo($id_arquivoT)
{
	...
	$sql->query("DELETE FROM tb_arquivos_documentos WHERE id_arquivoT  = :id_arquivoT ", [
		':id_arquivoT '=>$id_arquivoT
	]);
